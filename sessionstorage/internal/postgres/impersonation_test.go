...
func impersonationEnd(ctx context.Context, t *testing.T, q Queryer, id ccc.UUID) (endedAt *time.Time, endReason *string) {
	t.Helper()

	if err := q.QueryRow(ctx, `SELECT "EndedAt", "EndReason" FROM "SessionImpersonations" WHERE "SessionId" = $1`, id).Scan(&endedAt, &endReason); err != nil {
...
func TestSessionStorageDriver_EndImpersonation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "live record ends once", sessionID: impersonatedUserSession, reason: "Expired", wantReason: "Expired"},
		{name: "already ended record keeps its end", sessionID: impersonatedRoleSession, reason: "Revoked", wantReason: "Logout"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			...
			endedAt, endReason := impersonationEnd(ctx, t, conn.Pool, tt.sessionID)
			if endedAt == nil || endReason == nil || *endReason != tt.wantReason {
				t.Errorf("record end = (%v, %v), want (ended, %q)", endedAt, endReason, tt.wantReason)
			}
		})
	}
}
...
func TestSessionStorageDriver_EndImpersonation_NoRecordAndUnconfigured(t *testing.T) {
	...
	unconfigured := NewSessionStorageDriver(conn.Pool)
...
func TestSessionStorageDriver_DestroyImpersonatedSessions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, endReason := impersonationEnd(ctx, t, conn.Pool, tt.sessionID)
			gotReason := ""
			if endReason != nil {
				gotReason = *endReason
			}
...
func TestSessionStorageDriver_DestroySession_EndsImpersonation(t *testing.T) {
	...
	endedAt, endReason := impersonationEnd(ctx, t, conn.Pool, impersonatedUserSession)
	if endedAt == nil || endReason == nil || *endReason != "Logout" {
		t.Errorf("record end = (%v, %v), want (ended, Logout)", endedAt, endReason)
	}
...
func TestSessionStorageDriver_DestroyAllUserSessions_EndsImpersonation(t *testing.T) {
	...
	endedAt, endReason := impersonationEnd(ctx, t, conn.Pool, impersonatedUserSession)
	if endedAt == nil || endReason == nil || *endReason != "Revoked" {
		t.Errorf("record end = (%v, %v), want (ended, Revoked)", endedAt, endReason)
	}

	_, otherReason := impersonationEnd(ctx, t, conn.Pool, impersonatedCarol)
	if otherReason != nil {
...
func TestSessionStorageDriver_ActiveImpersonations(t *testing.T) {
	...
	if _, err := conn.Exec(ctx, `UPDATE "Sessions" SET "Expired" = TRUE WHERE "Id" = $1`, expiredSession); err != nil {
