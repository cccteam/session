...
func TestSessionStorageDriver_DestroyImpersonatedSession(t *testing.T) {
	...
	if err := NewSessionStorageDriver(conn.Client).DestroyImpersonatedSession(ctx, impersonatedCarol, string(sessioninfo.ImpersonationEndedByRevocation)); err == nil {
...
func TestSessionStorageDriver_UsernameKeyedOperations_LocalAndForeignActors(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	conn, err := prepareDatabase(ctx, t, "file://../../../schema/spanner/migrations", "file://../../../schema/spanner/impersonation/migrations")
	if err != nil {
		t.Fatalf("prepareDatabase() error = %v", err)
	}
	c := NewSessionStorageDriver(conn.Client)
