...
type userTestData struct {
	Locale     spanner.NullString `spanner:"Locale"`
	Theme      spanner.NullString `spanner:"Theme"`
	LoginCount spanner.NullInt64  `spanner:"LoginCount"`
}
...
type sessionUserRefData struct {
	UserIDRef spanner.NullString `spanner:"UserIdRef"`
}

func ns(s string) spanner.NullString {
...
func TestSessionStorageDriver_CreateUser_CustomUserData(t *testing.T) {
	...
	sourceURL := []string{"file://../../../schema/spanner/migrations", "file://testdata/users_test/custom_user_data"}
	...
	tests := []struct {
	...
	}{
		{
			...
			withConfig: true,
			customData: &userTestData{Locale: ns("fr-FR"), LoginCount: spanner.NullInt64{Int64: 1, Valid: true}},
			postAssertions: []string{
		...
		},
		{
			...
			withConfig: false,
			customData: &userTestData{Locale: ns("fr-FR")},
			wantErr:    true,
			postAssertions: []string{
...
func TestSessionStorageDriver_CustomUserData(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			withConfig: true,
			userID:     dataUserID,
			want:       &userTestData{Locale: ns("en-AU"), Theme: ns("dark"), LoginCount: spanner.NullInt64{Int64: 3, Valid: true}},
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			mutate: func(data any) error {
				...
				if typed.Locale.Valid || typed.Theme.Valid || typed.LoginCount.Valid {
					return errors.New("expected zero-value U for a user without a row")
				}
				typed.Locale = ns("de-DE")
...
func TestSessionStorageDriver_DeleteUser_CustomUserDataCascade(t *testing.T) {
	...
	ctx := t.Context()
	conn, err := prepareDatabase(ctx, t, "file://../../../schema/spanner/migrations", "file://testdata/users_test/custom_user_data")
	if err != nil {
		t.Fatalf("prepareDatabase() error = %v, wantErr %v", err, false)
	}
	c := NewSessionStorageDriver(conn.Client)
	...
	runAssertions(ctx, t, conn.Client, []string{`SELECT COUNT(*) = 1 FROM UserCustomData`})
	if err := c.DeleteUser(ctx, dataUserID); err != nil {
		t.Fatalf("SessionStorageDriver.DeleteUser() error = %v", err)
	}
	runAssertions(ctx, t, conn.Client, []string{`SELECT COUNT(*) = 0 FROM UserCustomData`})
}
...
func TestSessionStorageDriver_InsertSessionOIDC_OIDCUsers(t *testing.T) {
	...
	sourceURL := []string{"file://../../../schema/spanner/oidc/migrations", "file://testdata/sessions_test/oidc_users"}
	...
	oidcUserDataConfig := func(hook func(ctx context.Context, txn *spanner.ReadWriteTransaction, req *sessioninfo.NewSessionRequest, current any) (any, error)) *CustomUserDataConfig {
		return &CustomUserDataConfig{
	...
	tests := []struct {
		...
		},
		{
			...
			req:  sessioninfo.NewSessionRequest{Reason: sessioninfo.ReasonLogin, Username: "new@example.com", Tid: "tid-1", Oid: "oid-new"},
			userDataConfig: oidcUserDataConfig(func(_ context.Context, _ *spanner.ReadWriteTransaction, req *sessioninfo.NewSessionRequest, current any) (any, error) {
				if current != nil {
				...
				return &oidcUserTestData{Email: ns(req.Username)}, nil
			}),
			postAssertions: []string{
		...
		},
		{
			...
			userDataConfig: oidcUserDataConfig(func(_ context.Context, _ *spanner.ReadWriteTransaction, _ *sessioninfo.NewSessionRequest, _ any) (any, error) {
				return nil, nil
			}),
			postAssertions: []string{
				`SELECT COUNT(*) = 1 FROM OIDCUserData
		...
		},
		{
			...
			sessionConfig: &CustomSessionDataConfig{
				...
				Resolver: func(_ context.Context, _ *spanner.ReadWriteTransaction, req *sessioninfo.NewSessionRequest) (any, error) {
					...
					return &sessionUserRefData{UserIDRef: ns(req.UserID.String())}, nil
				},
			},
			postAssertions: []string{
				`SELECT COUNT(*) = 1 FROM SessionCustomData c JOIN OIDCUsers u ON c.UserIdRef = u.Id WHERE u.Oid = 'oid-new'`,
			},
			wantUserID: true,
		},
	}
...
func TestSessionStorageDriver_OIDCUserReads(t *testing.T) {
	...
	ctx := t.Context()
	conn, err := prepareDatabase(ctx, t, "file://../../../schema/spanner/oidc/migrations", "file://testdata/sessions_test/oidc_users")
	if err != nil {
		t.Fatalf("prepareDatabase() error = %v, wantErr %v", err, false)
	}
	c := NewSessionStorageDriver(conn.Client)
