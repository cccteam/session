...
func TestSessionStorageDriver_InsertSessionGoogleOIDC_GoogleOIDCUsers(t *testing.T) {
	...
	sourceURL := []string{"file://../../../schema/spanner/oidc-google/migrations", "file://testdata/sessions_test/google_oidc_users"}
	...
	googleUserDataConfig := func(hook func(ctx context.Context, txn *spanner.ReadWriteTransaction, req *sessioninfo.NewSessionRequest, current any) (any, error)) *CustomUserDataConfig {
	...
	tests := []struct {
		...
		},
		{
			...
			req:  sessioninfo.NewSessionRequest{Reason: sessioninfo.ReasonLogin, Username: "new@example.com", Sub: "sub-new", Hd: "example.com"},
			userDataConfig: googleUserDataConfig(func(_ context.Context, _ *spanner.ReadWriteTransaction, req *sessioninfo.NewSessionRequest, current any) (any, error) {
				if current != nil {
		...
		},
		{
			...
			userDataConfig: googleUserDataConfig(func(_ context.Context, _ *spanner.ReadWriteTransaction, _ *sessioninfo.NewSessionRequest, _ any) (any, error) {
		...
		},
		{
			...
			postAssertions: []string{
				`SELECT COUNT(*) = 1 FROM SessionCustomData c JOIN GoogleOIDCUsers u ON c.UserIdRef = u.Id WHERE u.Sub = 'sub-new'`,
			},
			wantUserID: true,
		},
	}
...
func TestSessionStorageDriver_GoogleOIDCUserReads(t *testing.T) {
	...
	ctx := t.Context()
	conn, err := prepareDatabase(ctx, t, "file://../../../schema/spanner/oidc-google/migrations", "file://testdata/sessions_test/google_oidc_users")
	if err != nil {
		t.Fatalf("prepareDatabase() error = %v, wantErr %v", err, false)
	}
	c := NewGoogleSessionStorageDriver(conn.Client)
